...
iran_server_configuration() {  
    ...
    #Add IPv6 Support
    local_ip='0.0.0.0'
    default_ipv6_answer="n"
    echo -ne "[-] Listen for IPv6 address? (y/n) [${YELLOW}default: $default_ipv6_answer${NC}]: "
    read -e -p "" answer
    answer=${answer:-$default_ipv6_answer}
    ...
    # Tunnel port
    default_tunnel_port="1075"
    while true; do
        echo -ne "[*] Tunnel port [${YELLOW}default: $default_tunnel_port${NC}]: "
        read -e -p "" tunnel_port
    ...
    while [[ "$nodelay" != "true" && "$nodelay" != "false" ]]; do
        echo -ne "[*] Enable TCP_NODELAY (true/false) [${YELLOW}default: $default_nodelay${NC}]: "
        read -e -p "" nodelay
        nodelay=${nodelay:-$default_nodelay}
    ...
    while [[ "$HEARTBEAT" != "true" && "$HEARTBEAT" != "false" ]]; do
        echo -ne "[*] Enable HEARTBEAT (true/false) [${YELLOW}default: $default_heartbeat${NC}]: "
        read -e -p "" HEARTBEAT
    ...
    while [[ "$transport" != "tcp" && "$transport" != "udp" ]]; do
        echo -ne "[*] Transport type (tcp/udp) [${YELLOW}default: $default_transport${NC}]: "
        read -e -p "" transport
...
kharej_server_configuration() {
    ...
    default_nodelay="true"
    while [[ "$nodelay" != "true" && "$nodelay" != "false" ]]; do
        echo -ne "[*] TCP_NODELAY (true/false) [${YELLOW}default: $default_nodelay${NC}]: "
        read -e -p "" nodelay
        nodelay=${nodelay:-$default_nodelay}
        if [[ "$nodelay" != "true" && "$nodelay" != "false" ]]; then
