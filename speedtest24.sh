...
if ! command -v speedtest > /dev/null 2>&1; then
    ...
    # Use the updated method for Ubuntu 24.04 compatibility
    curl -s https://packagecloud.io/install/repositories/ookla/speedtest-cli/script.deb.sh | sudo bash
    if [ $? -ne 0 ]; then
        echo "Failed to add speedtest-cli repository. Attempting to fix package system..."
        sudo apt update --fix-missing
        sudo apt install -f
        # Retry adding repository
        curl -s https://packagecloud.io/install/repositories/ookla/speedtest-cli/script.deb.sh | sudo bash
    fi
